...
class ChangeTrackerPlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        project.afterEvaluate {
            it.tasks
                .create(CHANGED_TRACKER_MODULES_TASK_NAME, ChangedModulesTask::class.java)
                .apply { group = CHANGED_TRACKER_GROUP_NAME }
    ...
    private fun getTaskForSubProjects(project: Project, rule: TaskRule): List<Task> {
        return project.subprojects.mapNotNull { subProject ->
            ...
            val containsTargetPlugin = rule.targetPlugin == null ||
                    subProject.pluginManager.hasPlugin(rule.targetPlugin)

            if (task != null && containsTargetPlugin) {
                task.apply {
                    onlyIf {
    ...
    private fun logAffectedProjects(project: Project, rule: TaskRule) {
        val result = getAffectedModulesForTask(project, rule)?.toMutableSet() ?: return

        if (rule.targetPlugin != null) {
            result.removeIf { subproject ->
                !subproject.pluginManager.hasPlugin(rule.targetPlugin)
            }
        }
